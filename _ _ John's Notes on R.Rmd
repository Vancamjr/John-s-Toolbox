...
mypkgs<-readr::read_tsv("/Users/John/Documents/R/mypkgs.tsv")         # Mac
# mypkgs<-readr::read_tsv("C:/Users/jvancamp/Documents/R/mypkgs.tsv") # Win
...
str_locate  #locates the first position of a pattern returns matrix of start & end
str_c #like paste0()
...
#ggplot2 changing legend title
p<-ggplot()+geom_other_stuff
p$labels$colour<-"Put Legend Title Here"
p  # to plot the result
...
# using theme to center align title & subtitle 
ggplot(trash,aes(date,value))+geom_line(col='red',size=1)+geom_point(col='red')+
  geom_line(aes(y=fitted),col='blue',size=1)+geom_point(aes(y=fitted),col='blue')+
  geom_line(aes(y=forecast),size=1)+geom_point(aes(y=forecast),col='black')+
  geom_ribbon(aes(ymin=lo80,ymax=hi80),alpha=.2)+
  geom_ribbon(aes(ymin=lo95,ymax=hi95),alpha=.1)+
  expand_limits(y=c(150000,350000))+
  theme_bw()+theme(plot.title=element_text(hjust=0.5),plot.subtitle=element_text(hjust=0.5))+
  labs(title=mytitle, subtitle=mysubtitle,x="Date",y="Sales Revenue")

# ggcl() "center labs" add this to a plot +ggcl() to center all labels except the caption
# ggcl<-function(){
